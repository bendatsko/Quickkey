...
function generateWordList(totalWords) {
  ...
  for (let i = 0; i < totalWords; i++) {
    let word = english[Math.floor(Math.random() * english.length)];
...
function populateWordList() {
  ...
  function changeLineTypical(availableSpace, charsUntilNextWord, wrapToken, i) {
    ...
    if (availableSpace < charsUntilNextWord) {
      ...
      for (let j = 0; i < wrapToken.length; j++) {
        lineCharCount++; //number of characters in the current line
        let lineStr = "t" + lineInt; //the name of the current line
        let th = document.createElement("TH"); //creates the next cell in the row
        document.getElementById(lineStr).appendChild(th); //adds the newest element to the current line
...
document.addEventListener("keydown", (event) => {
  if (textFocused === true) {
    ...
    if (keysPressed["Control"] && event.key == "Backspace") {
      ...
      for (let i = typedCharIndex; !foundSpace; i--) {
...
document.onkeydown = function () {
  ...
  if (textFocused === true) {
    ...
    if (typedCharIndex === wordList.length - 1) {
      ...
      let t1 = Date.now();
      let t1Date = new Date(t1);
      timestamp1 = t1Date.getTime();

      let totalTime = (timestamp1 - timestamp0) / 1000;
      ...
      let adjustedWPM = Math.round(rawWPM * percentageCharsCorrect);
...
let resetBtn = document.getElementById("reset-container");
